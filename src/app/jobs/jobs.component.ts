...
interface SelectedItems {'USER': string[], 'STAT': string[], 'QUEUE': string[], 'FROM_HOST': string[]};
...
export class JobsComponent implements OnInit {
	...
	@Input() filterFields = ['USER', 'STAT', 'QUEUE', 'FROM_HOST']; //the fields for which we want to filter
	@Input() columnLabels = {JOBID: 'Job ID', USER: 'User', STAT: 'Status', QUEUE: 'Queue', FROM_HOST: 'CE', EXEC_HOST: 'Exec host', CORES: 'Cores', JOB_NAME: 'Job name', SUBMIT_TIME: 'Submit time'};
	@Input() buttonData = [{label: 'Job details', url: '/job', field: 'JOBID'}, {label: 'Host details', url: '/host', field: 'EXEC_HOST'}];
	@Input() columnStyles = {FROM_HOST: "{'width':'200px'}"};
	@Input() private filepath = 'data/jobs/jobs.txt';
	@Input() private baseURL = '/jobs';
